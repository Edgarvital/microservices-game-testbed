...
\title{O Custo de Desempenho do mTLS em um \textit{Backend} de Microsservi\c{c}os sob Kubernetes: um Estudo Experimental Isolando \textit{Sidecar} e Criptografia}

\author{Edgar Vital\inst{1}}

\address{Programa de P\'os-Gradua\c{c}\~ao em Ci\^encia da Computa\c{c}\~ao\\
  Disciplina de Engenharia de Software Baseada em Evid\^encias --- Prof. Rodrigo Gusm\~ao de Carvalho Rocha
...
\begin{abstract}
The shift to Kubernetes-orchestrated microservices dissolves the traditional security perimeter, making \textit{mutual TLS} (mTLS) for east-west traffic a widely recommended countermeasure. Yet evidence on its \emph{performance cost} in realistic systems is scarce, and studies rarely separate how much of the cost comes from the service mesh sidecar versus the encryption itself. We report a controlled experiment measuring mTLS overhead on a microservices testbed representative of a multiplayer game backend (gRPC, REST, WebSocket, Redis). A three-treatment design --- no mesh, mesh without mTLS, and mesh with strict mTLS --- \emph{isolates} the sidecar cost from the encryption cost. In a local environment (kind + Istio), with eight repetitions per treatment, the sidecar accounts for most of the degradation, while enabling mTLS on top of an existing mesh costs only about 5\% in latency and throughput. Once a mesh is adopted, turning on encryption is cheap.
\end{abstract}

\begin{resumo}
A migra\c{c}\~ao para microsservi\c{c}os orquestrados por Kubernetes dissolve o per\'imetro de seguran\c{c}a tradicional, tornando o \textit{mutual TLS} (mTLS) no tr\'afego \textit{east-west} uma contramedida amplamente recomendada. Contudo, h\'a escassez de evid\^encias sobre seu \emph{custo de desempenho} em sistemas realistas, e os estudos raramente separam quanto do custo advem do \textit{sidecar} do \textit{service mesh} e quanto advem da criptografia em si. Relatamos um experimento controlado que mede o \textit{overhead} do mTLS em um \textit{testbed} de microsservi\c{c}os representativo do \textit{backend} de um jogo multiplayer (gRPC, REST, WebSocket, Redis). Um desenho de tr\^es tratamentos --- sem \textit{mesh}, \textit{mesh} sem mTLS e \textit{mesh} com mTLS \textit{STRICT} --- \emph{isola} o custo do \textit{sidecar} do custo da criptografia. Em ambiente local (\textit{kind} + Istio), com oito repeti\c{c}\~oes por tratamento, o \textit{sidecar} responde pela maior parte da degrada\c{c}\~ao, enquanto habilitar o mTLS sobre o \textit{mesh} j\'a presente custa apenas cerca de 5\% em lat\^encia e vaz\~ao. Uma vez adotado o \textit{mesh}, ativar a criptografia \'e barato.
\end{resumo}
...
\textbf{Defesas criptogr\'aficas em Kubernetes.} Fernandez e Brito \cite{fernandez2019} prop\~oem um orquestrador que integra Intel SGX e medem \textit{overhead} expressivo em opera\c{c}\~oes cr\'iticas (por exemplo, $2{,}37\times$ em \textit{auto-scaling}). Falc\~ao et al.\ \cite{falcao2025} avaliam modelos de \textit{deployment} confidencial (Intel TDX, AMD SEV-SNP) em tr\^es provedores de nuvem, evidenciando \textit{trade-offs} de desempenho entre arquiteturas. Stoyanov et al.\ \cite{stoyanov2024} tratam de criptografia fim-a-fim para \textit{checkpoint/restore} de \textit{containers}, mostrando que otimiza\c{c}\~oes podem reduzir o \textit{overhead} da cifragem em at\'e $62\%$. Esses trabalhos motivam nosso foco no custo da criptografia, mas atuam em camadas distintas (\textit{enclaves}, \textit{snapshots}), n\~ao no tr\'afego \textit{east-west} via \textit{mesh}.

\textbf{Seguran\c{c}a de microsservi\c{c}os.} Flora e Antunes \cite{flora2024} prop\~oem um \textit{benchmark} e \textit{dataset} para detec\c{c}\~ao de intrus\~ao em microsservi\c{c}os, refor\c{c}ando a necessidade de avalia\c{c}\~oes emp\'iricas padronizadas. Baarzi et al.\ \cite{baarzi2020} defendem microsservi\c{c}os contra ataques de nega\c{c}\~ao de servi\c{c}o via \textit{fissioning} adaptativo. Tais estudos endere\c{c}am a face de \emph{efic\'acia} da seguran\c{c}a, complementar \`a face de \emph{custo} investigada aqui.

\textbf{Overhead de \textit{service mesh} e mTLS.} Zhu et al.\ \cite{zhu2023}, com a ferramenta MeshInsight, dissecam o \textit{overhead} de \textit{sidecars} e reportam aumentos de lat\^encia de at\'e $269\%$ e de CPU de at\'e $163\%$, atribuindo o custo dominante ao reprocessamento de protocolo. Bremler-Barr et al.\ \cite{bremlerbarr2024} comparam \textit{frameworks} e medem, ao habilitar mTLS, aumentos de lat\^encia de $166\%$ (Istio com \textit{sidecar}), $33\%$ (Linkerd) e $8\%$ (Istio Ambient). Nosso trabalho difere ao (i) \emph{separar} explicitamente o custo do \textit{sidecar} do custo do mTLS por meio de um tratamento intermedi\'ario e (ii) medir sobre protocolos heterog\^eneos em um sistema de aplica\c{c}\~ao realista, em vez de um \textit{echo server} sint\'etico.
